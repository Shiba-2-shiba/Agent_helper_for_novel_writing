...
ここでは、10万字の執筆をゴールに見すえ、この環境をどう活用していくかをステップ・バイ・ステップで解説します。
...
```bash
python scripts/init_project.py [あなたの小説のタイトル]
# 例: python scripts/init_project.py my_first_novel
```

これを実行すると、指定したタイトルのフォルダが作られ、中に **5つの設定テンプレート** と **第1章〜第5章のフォルダ** が自動生成されます。
...
5. [05_chapter_outline_100k.md](templates/05_chapter_outline_100k.md): **10万字達成のための最大の武器**です。ここを見ながら、「第1章にはシーンがいくつあるか」「今はどのシーンを書いているか」を管理します。

## ステップ 4: 本文の執筆 ＆ ローカルLLMの活用 (Writing & AI Prompting)
章フォルダ（例: `chapter_1_introduction/body.md`）に実際の本文を書いていきます。
...
> [!TIP]
> **10万字を書き切るコツ**
> 「今日は第1章のシーン3（2500文字）だけを書く」というように、[05_chapter_outline_100k.md](templates/05_chapter_outline_100k.md) で設定した小さなマイルストーンを日々クリアしていくことを意識すると、途中で挫折せずに完結までたどり着きやすくなります！
